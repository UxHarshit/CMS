...
export default function ProblemPage(props: {
  ...
  useEffect(() => {
    const initialSavedCodes = questions.reduce((acc, question) => {
      acc[question.id] = languages.reduce((langAcc, lang) => {
        langAcc[lang.value] =
          starterCode[lang.value as keyof typeof starterCode];
        return langAcc;
  ...
  const currentCode =
    savedCodes[currentQuestionIdx.id]?.[language] ||
  ...
  const handleRunCode = async () => {
    ...
    const response = await fetch(`${props.baseUrl}/api/problems/run`, {
      ...
      body: JSON.stringify({
        ...
        language_id: languages.find((lang) => lang.value === language)
          ?.language_id,
        code: currentCode,
        stdin: input,
      }),
    ...
    if (response.ok) {
      ...
      if (message == "Compilation Error") {
        ...
        const decoded = atob(compile_output)
          .replace(/â/g, "")
          .replace(/â/g, "");
    ...
    await fetch(`${props.baseUrl}/api/problems/submit`, {
      ...
      .then(async (res) => {
        ...
        if (res.ok) {
          ...
          let reverseIndex = entries.findIndex(
            ([key, value]) => value.status === 3
          );
  ...
  const handleQuestionChange = (idx: number) => {
    ...
    setCode(
      savedCodes[questions[idx].id]?.[language] ||
        starterCode[language as keyof typeof starterCode]
    );
    setError([]);
  };
